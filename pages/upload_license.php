...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['doc_licencia_back']['name'])) {
        $ext = pathinfo($_FILES['doc_licencia_back']['name'], PATHINFO_EXTENSION);
    ...
    header('Location: profile.php?toast=doc_licencia&tab=documentos');
    exit;
}
...
<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; padding: 10px 0;">
    ...
    <span style="font-weight: 800; font-size: 16px; color: var(--text);">Licencia</span>
...
<div class="card" style="border:none; padding: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.02); border-radius: 24px; background: #fff; margin-bottom: 100px;">
    <h2 style="font-size: 20px; font-weight: 800; color: var(--text); margin-top: 0; margin-bottom: 6px;">Registro de Conducir</h2>
    <p style="font-size: 13.5px; color: var(--muted); font-weight: 600; margin: 0 0 24px 0; line-height: 1.4;">
        Sube capturas nítidas y legibles de tu Registro de Conducir (licencia de conducir) vigente.
    </p>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-group" style="margin-bottom: 24px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-front').click()" style="cursor: pointer; position: relative;">
                <input type="file" name="doc_licencia_front" id="file-front" accept="image/*" style="display: none;" onchange="previewImage(this, 'preview-front', 'placeholder-front')">
                
                <div class="preview-container" id="preview-front" style="display: <?= !empty($userData['doc_licencia_path']) ? 'block' : 'none' ?>; text-align: center;">
                    <img src="<?= !empty($userData['doc_licencia_path']) ? esc(protected_file_url($userData['doc_licencia_path'])) : '' ?>" style="width: 100%; max-height: 180px; object-fit: contain; border-radius: 16px; border: 1px solid var(--border); padding: 5px; background: #fafafa;">
                    <div style="font-size: 11px; color: var(--muted); font-weight: 700; margin-top: 8px;">Haz clic para cambiar la foto</div>
                </div>
                
                <div class="upload-placeholder" id="placeholder-front" style="display: <?= !empty($userData['doc_licencia_path']) ? 'none' : 'flex' ?>; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; border: 2px dashed var(--border); border-radius: 20px; background: rgba(0,0,0,0.01); transition: all 0.2s;">
        ...
        <div class="form-group" style="margin-bottom: 30px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-back').click()" style="cursor: pointer; position: relative;">
                <input type="file" name="doc_licencia_back" id="file-back" accept="image/*" style="display: none;" onchange="previewImage(this, 'preview-back', 'placeholder-back')">
                
                <div class="preview-container" id="preview-back" style="display: <?= !empty($userData['doc_licencia_back_path']) ? 'block' : 'none' ?>; text-align: center;">
                    <img src="<?= !empty($userData['doc_licencia_back_path']) ? esc(protected_file_url($userData['doc_licencia_back_path'])) : '' ?>" style="width: 100%; max-height: 180px; object-fit: contain; border-radius: 16px; border: 1px solid var(--border); padding: 5px; background: #fafafa;">
                    <div style="font-size: 11px; color: var(--muted); font-weight: 700; margin-top: 8px;">Haz clic para cambiar la foto</div>
                </div>
                
                <div class="upload-placeholder" id="placeholder-back" style="display: <?= !empty($userData['doc_licencia_back_path']) ? 'none' : 'flex' ?>; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; border: 2px dashed var(--border); border-radius: 20px; background: rgba(0,0,0,0.01); transition: all 0.2s;">
                    <span style="font-size: 36px; margin-bottom: 8px;">📸</span>
