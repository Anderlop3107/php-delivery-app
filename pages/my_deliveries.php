...
<style>
    ...
    .tracking-bottom-sheet {
        position: fixed !important; bottom: 50px !important; top: auto !important; left: 25px !important; right: 25px !important;
        ...
        ?>
            <div class="status-card <?= $cardStateClass ?>" id="card-<?= $row['id'] ?>">
                <?php if ($isDriver && count($activeRows) > 1 && isset($row['sequence_number'])): ?>
                    <div class="status-top" style="margin-bottom: 10px;">
                        <span style="font-size: 11px; font-weight: 800; color: var(--primary); background: var(--primary-soft); padding: 4px 8px; border-radius: 6px; display: inline-block; width: max-content; letter-spacing: 0.5px;">
                            📍 PARADA <?= $row['sequence_number'] ?>
                        </span>
                    </div>
                <?php endif; ?>

                <!-- 1. ENCABEZADO DE REPARTIDOR / LOCAL (SIN CAJA INTERNA) -->
                <?php if ($isLocal && !empty($row['repartidor_name'])): ?>
                    <div class="assigned-driver-header" onclick='openTrackingSheetModal(<?= json_encode($row) ?>)' style="display: flex; align-items: center; justify-content: space-between; gap: 14px; padding-bottom: 14px; border-bottom: 1px solid rgba(226, 232, 240, 0.8); cursor: pointer;">
                ...
                <?php else: ?>
                    <div style="display: flex; align-items: center; gap: 14px; padding-bottom: 14px; border-bottom: 1px solid rgba(226, 232, 240, 0.8);">
                        <?php if ($isLocal && empty($row['repartidor_name'])): ?>
                            <div class="person-avatar searching-avatar" style="width: 44px; height: 44px;">
                                <div class="searching-radar-ring"></div>
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--primary, #2563eb)" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="animation: pulse-dot 1.5s infinite; position: relative; z-index: 2;">
                            ...
                            <div class="person-details">
                                <b style="color: var(--primary, #2563eb); display: flex; align-items: center; gap: 6px; font-size: 14px;">
                                    Buscando repartidor...
                                </b>
                                <span style="font-size: 12px; color: var(--muted);">Búsqueda en tiempo real activa</span>
                            </div>
                        <?php else: ?>
                            <div class="person-avatar" style="width: 44px; height: 44px; border-radius: 12px; font-size: 20px; overflow: hidden; background: #f1f5f9; display: flex; align-items: center; justify-content: center;">
                            ...
                            <div class="person-details" style="flex: 1;">
                                <b style="font-size: 14.5px; color: var(--text); display: block;"><?= esc($row['local_name']) ?></b>
                                <span style="font-size: 12px; color: var(--muted); display: block;">Punto de retiro</span>
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>

                <!-- 2. DETALLES DE LA ENTREGA Y CLIENTE (INTEGRADOS DIRECTAMENTE SIN CAJAS) -->
                <div class="customer-info <?= $ocultarCliente ? 'oculto' : '' ?>" id="info-cliente-<?= $row['id'] ?>" style="margin-top: 14px; display: flex; flex-direction: column; gap: 12px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 6px; padding-bottom: 12px; border-bottom: 1px solid rgba(226, 232, 240, 0.8);">
                        ...
                        <!-- Cliente y Botones de Contacto (WhatsApp / Llamada) -->
                        <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-top: 6px; padding-left: 42px;">
