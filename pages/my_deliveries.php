    ...
    <?php else: ?>
        ...
        ?>
            <div class="status-card <?= $cardStateClass ?>" id="card-<?= $row['id'] ?>">
                ...
                <!-- BLOQUE DEL LOCAL / REPARTIDOR (AL PRINCIPIO DE LA TARJETA) -->
                <?php if ($isLocal && !empty($row['repartidor_name'])): ?>
                    <!-- Tarjeta de Repartidor Asignado (Clic abre Seguimiento en Vivo con Icono >) -->
                    <div class="person-box assigned-box <?= $ocultarLocal ? 'oculto' : '' ?>" id="info-local-<?= $row['id'] ?>" onclick='openTrackingSheetModal(<?= json_encode($row) ?>)' style="margin-top: 0; margin-bottom: 16px;">
                ...
                <?php else: ?>
                    ...
                    <div class="person-box <?= $ocultarLocal ? 'oculto' : '' ?>" id="info-local-<?= $row['id'] ?>" style="margin-top: 0; margin-bottom: 16px;">
                        <?php if ($isLocal && empty($row['repartidor_name'])): ?>
                ...
                <!-- BLOQUE DEL CLIENTE -->
                <div class="customer-info <?= $ocultarCliente ? 'oculto' : '' ?>" id="info-cliente-<?= $row['id'] ?>" style="margin-top: 0; display: flex; flex-direction: column; gap: 10px;">
