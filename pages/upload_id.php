...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($uploadedAny) {
        app_exec("UPDATE users SET status_doc_ci = 'pending' WHERE id = ?", 'i', [(int)$user['id']]);
    }
    
    header('Location: profile.php?toast=doc_ci&tab=documentos');
    exit;
}
        ...
        <div class="form-group" style="margin-bottom: 24px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-front').click()" style="cursor: pointer; position: relative;">
                ...
                <div class="preview-container" id="preview-front" style="display: <?= !empty($userData['doc_ci_path']) ? 'block' : 'none' ?>; text-align: center;">
                    <img src="<?= !empty($userData['doc_ci_path']) ? esc(protected_file_url($userData['doc_ci_path'])) : '' ?>" style="width: 100%; max-height: 180px; object-fit: contain; border-radius: 16px; border: 1px solid var(--border); padding: 5px; background: #fafafa;">
        ...
        <div class="form-group" style="margin-bottom: 30px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-back').click()" style="cursor: pointer; position: relative;">
                ...
                <div class="preview-container" id="preview-back" style="display: <?= !empty($userData['doc_ci_back_path']) ? 'block' : 'none' ?>; text-align: center;">
                    <img src="<?= !empty($userData['doc_ci_back_path']) ? esc(protected_file_url($userData['doc_ci_back_path'])) : '' ?>" style="width: 100%; max-height: 180px; object-fit: contain; border-radius: 16px; border: 1px solid var(--border); padding: 5px; background: #fafafa;">
                    <div style="font-size: 11px; color: var(--muted); font-weight: 700; margin-top: 8px;">Haz clic para cambiar la foto</div>
                </div>
