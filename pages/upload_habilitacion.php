...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($uploadedAny) {
        app_exec("UPDATE users SET status_doc_habilitacion = 'pending' WHERE id = ?", 'i', [(int)$user['id']]);
    }
    
    header('Location: profile.php?toast=doc_habilitacion&tab=documentos');
    exit;
}

$title = 'Habilitación de Vehículo';
...
<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; padding: 10px 0;">
    ...
    <span style="font-weight: 800; font-size: 16px; color: var(--text);">Habilitación</span>
...
<div class="card" style="border:none; padding: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.02); border-radius: 24px; background: #fff; margin-bottom: 100px;">
    <h2 style="font-size: 20px; font-weight: 800; color: var(--text); margin-top: 0; margin-bottom: 6px;">Habilitación de Vehículo</h2>
    <p style="font-size: 13.5px; color: var(--muted); font-weight: 600; margin: 0 0 24px 0; line-height: 1.4;">
        Sube capturas nítidas y legibles de tu Habilitación de Vehículo correspondiente a tu medio de transporte.
    </p>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-group" style="margin-bottom: 24px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-front').click()" style="cursor: pointer; position: relative;">
                ...
                <div class="upload-placeholder" id="placeholder-front" style="display: <?= !empty($userData['doc_habilitacion_path']) ? 'none' : 'flex' ?>; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; border: 2px dashed var(--border); border-radius: 20px; background: rgba(0,0,0,0.01); transition: all 0.2s;">
        ...
        <div class="form-group" style="margin-bottom: 30px;">
            ...
            <div class="upload-zone" onclick="document.getElementById('file-back').click()" style="cursor: pointer; position: relative;">
                ...
                <div class="upload-placeholder" id="placeholder-back" style="display: <?= !empty($userData['doc_habilitacion_back_path']) ? 'none' : 'flex' ?>; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; border: 2px dashed var(--border); border-radius: 20px; background: rgba(0,0,0,0.01); transition: all 0.2s;">
                    <span style="font-size: 36px; margin-bottom: 8px;">📸</span>
